...
export function CDPMap({ onPlotDataChange, initialLat, initialLng, initialZoom }: CDPMapProps) {
  const mapRef = useRef<any>(null)
  const mapContainerRef = useRef<HTMLDivElement>(null)
  const [isMapReady, setIsMapReady] = useState(false)
  const [drawingMode, setDrawingMode] = useState<"freehand" | "polygon" | null>(null)
  ...
  const [floorHeights, setFloorHeights] = useState({ h1: "3.0", h2: "3.3", h3: "3.5", h4: "3.75", h5: "3.8" })
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      console.log("Initializing map at:", centerLat, centerLng, "zoom:", zoomLevel)
    ...
    try {
      ...
      geojson.features.forEach((feature: any, index: number) => {
        if (feature.geometry?.type === "Point") {
          ...
          marker.bindPopup(`<strong>Survey ${name}</strong><br/>Lat: ${lat.toFixed(6)}<br/>Lng: ${lng.toFixed(6)}`)
          marker.addTo(map)
      ...
      geojson.features.forEach((feature: any) => {
        if (
          ...
          const strokeColor = feature.properties?.stroke || feature.properties?.color || "#dc2626"
          const fillColor = feature.properties?.fill || strokeColor
          const fillOpacity = feature.properties?.['fill-opacity'] !== undefined ? feature.properties?.['fill-opacity'] : 0.3
          const strokeOpacity = feature.properties?.['stroke-opacity'] !== undefined ? feature.properties?.['stroke-opacity'] : 1
          const strokeWidth = feature.properties?.['stroke-width'] || feature.properties?.weight || 3
  ...
  const startDrawing = async (mode: "freehand" | "polygon") => {
    ...
    if (mode === "freehand") {
      ...
      const onMouseDown = () => {
        ...
        previewLine = L.polyline([], { color: "#2563eb", weight: 2, opacity: 0.7 })
      ...
      const onMouseUp = () => {
        if (isDrawing && freehandPolygon.length > 2) {
          ...
          const polygon = L.polygon(freehandPolygon, { color: "#2563eb", fillOpacity: 0.2 })
      ...
      const onClick = (e: any) => {
        ...
        // Add preview marker
        const marker = L.circleMarker([e.latlng.lat, e.latlng.lng], { radius: 4, color: "#2563eb", fillOpacity: 0.8 })
        ...
        previewLine = L.polyline(polygonPoints, { color: "#2563eb", weight: 2, opacity: 0.7 })
        previewLine.addTo(mapRef.current)
      }
      ...
      const onDblClick = (e: any) => {
        ...
        if (polygonPoints.length > 2) {
          ...
          const polygon = L.polygon(polygonPoints, { color: "#2563eb", fillOpacity: 0.2 })
          polygon.addTo(mapRef.current)
          setDrawnLayer(polygon)
          calculateArea(polygon)
          mapRef.current.dragging.enable()
  ...
  const clearDrawing = () => {
    ...
    if (drawnLayer) {
      try { map.removeLayer(drawnLayer) } catch {}
      setDrawnLayer(null)
    }
    ...
    map.eachLayer((layer: any) => {
      ...
      if (layer instanceof (window as any).L.Polyline && !layer._popup) {
        if (layer.options.color === "#2563eb" && !drawnLayer?.contains?.(layer)) {
          try { map.removeLayer(layer) } catch {}
        }
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <Button onClick={handleSearch} className="bg-blue-600 hover:bg-blue-700 btn-animate gap-2">
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          ...
          className={`gap-2 ${drawingMode === "freehand" ? "bg-blue-600" : "border-blue-500 text-blue-600 hover:bg-blue-600 hover:text-white"} btn-outline-animate`}
        ...
        <Button
          ...
          className={`gap-2 ${drawingMode === "polygon" ? "bg-blue-600" : "border-blue-500 text-blue-600 hover:bg-blue-600 hover:text-white"} btn-outline-animate`}
      ...
      {drawingMode && (
        <div className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">
          {drawingMode === "freehand" && "🖱️ Click and drag to draw freehand (map is locked while drawing)"}
          {drawingMode === "polygon" && "🖱️ Click to add points, double-click to finish (map is locked while drawing)"}
        </div>
      )}

      {/* Map Container */}
      <div className="flex items-center gap-3 mb-2">
        <label className="text-sm"><input type="checkbox" checked={showKMZ} onChange={() => setShowKMZ((v) => !v)} className="mr-2"/>Show KMZ overlay (legends)</label>
      </div>
      <div ref={mapContainerRef} className="w-full h-[500px] rounded-lg border-2 border-blue-200 shadow-lg" />

      {/* Input Fields */}
      <Card className="p-4 bg-blue-50 border-blue-200">
        <h3 className="font-semibold text-blue-900 mb-3">Site Details & Parameters</h3>

        <div className="grid md:grid-cols-3 gap-4 mb-4">
          <div>
            <Label className="text-sm font-semibold text-blue-900">Zone Type</Label>
            <Select value={selectedZone} onValueChange={setSelectedZone}>
              ...
              <SelectContent>
                ...
                <SelectItem value="residential_main">Residential Main</SelectItem>
                <SelectItem value="residential_mixed">Residential Mixed</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div>
            <Label className="text-sm font-semibold text-blue-900">Road Width (m)</Label>
          ...
          <div>
            <Label className="text-sm font-semibold text-blue-900">Site Area (sq m)</Label>
            <Input
              type="number"
              value={siteArea}
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <div>
            <Label className="text-sm font-semibold text-blue-900">Plot Width (m)</Label>
            <Input
              type="number"
              value={plotWidth}
          ...
          <div>
            <Label className="text-sm font-semibold text-blue-900">Plot Depth (m)</Label>
            <Input
              type="number"
              value={plotDepth}
          ...
          <div className="grid grid-cols-5 gap-2">
            {Object.keys(floorHeights).map((key, idx) => (
              <Input
                ...
                onChange={(e) => setFloorHeights({ ...floorHeights, [key]: e.target.value })}
                placeholder={`H${idx + 1}`}
        ...
        <Button onClick={handleCalculate} className="w-full mt-4 bg-blue-600 hover:bg-blue-700 btn-animate">
